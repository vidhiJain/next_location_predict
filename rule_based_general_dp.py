...
# filename = 'yig0604_0.txt'  # 'zi0619_0.txt'
file_list = glob(Path(root, messages, '*_0.txt').as_posix())
...
for filename in file_list:
    ...
    general_dp = pd.read_csv(Path(root, dp, filename.split('.')[0]+'_general_dp.csv'))
    ...
    for entry in data:
        if entry['header']['message_type']=='observation':
            ...
            if float(entry['header']['timestamp']) >= general_dp.iloc[general_dp_counter]['timestamp']:
                # breakpoint()
                next_room_gt = (general_dp.iloc[general_dp_counter]['nextRoom'], general_dp.iloc[general_dp_counter]['nextHallway'])
                record.append([index_name_rooms[str(probmax)], next_room_gt])
                general_dp_counter += 1
    ...
    def accuracy(record):
        ...
        for entry in record:
            if entry[0] == entry[1][0] or entry[0] == entry[1][1]:
                correct += 1
